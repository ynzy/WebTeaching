...
# 绿色框表示 自己弄懂得,`随时记得标点符号是英文状态。`
  ...
  * https://www.w3school.com.cn/css/css_background.asp
    ...
    * -100px;是相反的方向
    * opacity: 0.6; 透明度
    * border-radius: 10px; 如果是正方形变成圆就用50%,如果是长方形就用像素。
...
# 不理解
*  outline: none;/* 谷歌哟焦点外边框，去掉焦点外边框 */
*  cursor: pointer; 鼠标上上去有手型

# 一开始弄错得
   1. background-image: url(../img/search.jpg) no-repeat; 这种写法会使图片消失，因为语法错误。
  `background: url(../img/search.jpg) no-repeat;`
   或者是
  ```background:url(../img/search.jpg);background-repeat: no-repeat;
    background-position: center;
   ```
   2. <div `class="search"`><div class="center"></div></div>应该改成 <div` id="search"`><div class="center"></div></div>,一般最外面一层都用id表示。
   3.  <button class="search">搜索</button> 应该是`<input type="text" class="search" placeholder="请输入旅游景点或城市"> `中的class 设置css
...
# 一开始弄错得
1. 父元素不是.center  应该是#search
2. opacity: 0.6; 会使子元素都变透明
...
```
  ** .center 和input  button 并没有父子关系 **
  1. .`link已经不再是ul了，那么link 就灭有了可以去除li前面小点得权限，必须要重新写ul{ list-style:outside none none;}`
    如下
```
    <!DOCTYPE html>
<html>
...
```
box-shadow: 0 8px 10px rgba(0,0,0,0.2);设置阴影
position: relative;
z-index:9999; 权限最高；
